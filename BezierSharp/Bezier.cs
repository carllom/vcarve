...
namespace vcarve.BezierSharp
{
    ...
    /// C# partial port of http://pomax.github.io/bezierjs
    /// </summary>
    class Bezier
    {
        Point[] points; // point array. Start, Control1, [Control2], End
        int dimlen = 2; // number of dimensions
        bool _linear; // Is the curve a line
        TPoint[] _lut = Array.Empty<TPoint>(); // Lookup table for points on the curve
        int _t1 = 0; // Parameter range: start
        int _t2 = 1; // Parameter range: end

        IEnumerable<Point[]> dpoints; // Derivative points
        bool clockwise;
        string _print; // digest of the point coordinates
        double[] ratios = Array.Empty<double>(); // ratios of arc length to straight line length

        public Bezier(Point begin, Point end, Point c)
        {
            points = new Point[] { begin, c, end };
            var aligned = BezierUtils.Align(points, new LineSegment(begin, end));
            var baselength = BezierUtils.Dist(begin, end);
            _linear = aligned.Aggregate(0D, (t, p) => t + Math.Abs(p.y)) < baselength / 50;
            Update();
        }

        public Bezier(Point begin, Point end, Point c1, Point c2)
        {
            points = new Point[] { begin, c1, c2, end };
        ...
        private void Update() {
            ...
            _lut = Array.Empty<TPoint>();
            dpoints = BezierUtils.Derive(points);
            ComputeDirection();
        }

        private void ComputeDirection() {
            var angle = BezierUtils.Angle(points[0], points[^1], points[1]);
            clockwise = angle > 0;
        }

        /// <summary>
        /// Calculates the length of this Bezier curve.
        /// Length is calculated using numerical approximation, specifically the Legendre-Gauss quadrature algorithm.
        /// </summary>
        /// <returns></returns>
        public double Length() {
            return BezierUtils.Length(Derivative);
        }

        /// <summary>
        /// Calculates the curve tangent at the specified t value.
        /// Note that this yields a not-normalized vector {x: dx, y: dy}.
        /// </summary>
        /// <param name="t"></param>
        /// <returns>Tangent vector</returns>
        public TPoint Derivative(double t) {
            return BezierUtils.Compute(t, dpoints.First());
        }


        public TPoint Normal(double t) {
            var d = Derivative(t);
        ...
        /// <summary>
        /// Generates a LookUp Table of coordinates on the curve, spaced at parametrically equidistance intervals.
        /// If steps is given, the LUT will contain steps+1 coordinates representing the coordinates from t=0 to t=1 at interval 1/steps.
        ...
        /// </summary>
        /// <param name="t"></param>
        /// <returns>Coordinate for the point on the curve</returns>
        public TPoint Compute(double t) {
            if (ratios.Length > 0) {
                return BezierUtils.ComputeWithRatios(t, points, ratios);
            }
            return BezierUtils.Compute(t, points);
        }
        ...
        public (TPoint point, double distance)  Project(Point point) {
            ...
            for (; t < t2 + step; t += step) {
                p = Compute(t);
                var d = BezierUtils.Dist(point, p);
                if (d < mdist) {
                    mdist = d;
                    ft = t;
                }
...
// Curves are defined as as start point, control point 1, (optional) control point 2, end point,...
